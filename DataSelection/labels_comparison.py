...
class LabelManager:
    ...
    counter = 0
    return_flag = False
    columns_names = ["file_name", "selected site", "corrected site"]
    result = pd.DataFrame(columns=columns_names)
    close_flag = False

    def iterate(self):
        self.image_picker()
        corrected_site = display(INPUT_PATH + "\\" + self.buffer_s[self.counter] + "\\" + self.buffer_f[self.counter], self.counter)
        if corrected_site == 'return':
            ...
            self.result = self.result.append(pd.Series([self.buffer_f[self.counter], self.buffer_s[self.counter],
                                                        corrected_site],
                                                       index=self.columns_names), ignore_index=True)
            self.counter += 1

    def check_picker(self, s, f):
        """
        ...
        :param f: file_name
        """
        while ((self.result["file_name"] == f) & (
                self.result["selected site"] == s)).any():
            self.image_picker()

    def array_append(self, s, f):
    ...
    def image_picker(self):
        ...
        picked_frame = random.choice(os.listdir(INPUT_PATH + "\\" + picked_site))

        self.array_append(picked_site, picked_frame)
