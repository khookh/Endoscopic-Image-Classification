...
def heatmap_from_pred(x):
    with tf.GradientTape() as tape:
        last_conv_layer = densenet121.get_layer('relu')#('conv5_block16_concat')#('conv5_block16_2_conv')
        iterate = tf.keras.models.Model([densenet121.inputs], [densenet121.output, last_conv_layer.output])
    ...
    while True:
        ...
        while True:
            ...
            if k == ord('q'):
                ...
                break
            if k == ord('s'):
                p = OUTPUT_PATH+"\\"+picked_site+"_"+site+".png"
                cv.imwrite(p,hm)
